...
public function edit(Attendance $attendance)
{
    ...
    // Get QR codes for this class - TAMBAHKAN INI
...
/**
 * Dashboard Absensi Siswa - DIPERBAIKI
 */
/**
...
/**
 * Detail absensi siswa
 */// Di dalam AttendanceController class, tambahkan method ini:
...
// Kemudian di view, Anda bisa mengakses melalui controller method
// Atau buat accessor di model Attendance
 ...
 */
// AttendanceController.php - Tambahkan method scanPage():
...
public function processScan($code, Request $request)
{
    ...
    try {
        ...
        if (!$qrCode->is_active_now) {
            return redirect()->back()->with('error', 'QR Code belum aktif atau sudah berakhir.');
        }
        ...
        if ($qrCode->location_restricted) {
            ...
            if (!$latitude || !$longitude) {
                return redirect()->back()->with('error', 'Lokasi diperlukan untuk QR Code ini.');
            }
            
            if (!$qrCode->isLocationWithinRadius($latitude, $longitude)) {
                return redirect()->back()->with('error', 'Anda berada di luar radius yang ditentukan.');
            }
        }
...
// AttendanceController.php - tambahkan method berikut

/**
 * Proses absensi dari scan QR Code (untuk semua siswa)
 */
    ...
    try {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'time' => $attendance->formatted_checked_in_at,
...
public function apiScan(Request $request)
{
    ...
    return response()->json([
        ...
        'data' => [
            ...
            'class_id' => $qrCode->class_id,
            'class_name' => $qrCode->class->class_name,
            'teacher_name' => $qrCode->class->teacher->name,
        ...
        if ($existingAttendance) {
            return response()->json([
                ...
                'data' => [
                    ...
                    'time' => $existingAttendance->formatted_checked_in_at
                ]
            ], 400);
        }
    ...
    $students = $qrCode->class->students()
        ...
        ->map(function($student) {
            ...
            return [
                ...
                'attendance_status' => $attendance ? $attendance->status_text : 'Belum Absen',
                'attendance_time' => $attendance ? $attendance->formatted_checked_in_at : null,
                'status_color' => $attendance ? $attendance->status_color : 'secondary',
            ];
        });
    ...
    return response()->json([
        ...
        'data' => [
            'qr_code' => [
                ...
                'time_range' => $qrCode->formatted_time_range,
                'time_remaining' => $qrCode->time_remaining,
                'is_active' => $qrCode->is_active_now,
            ],
            'students' => $students,
            'statistics' => [
...
// AttendanceController.php - Perbaiki method quickGenerateQr

/**
 * Quick generate QR Code untuk kelas hari ini
 */
public function quickGenerateQr($classId)
{
    $user = Auth::user();
    ...
    try {
        ...
        // Generate QR code image - GUNAKAN LIBRARY YANG TERSEDIA
